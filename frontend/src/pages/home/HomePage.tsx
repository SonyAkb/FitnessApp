...
import { Lottie, LottieDisplay, useLottie } from 'lottie-react'

import happyAnim from '../../assets/Sample Animation.json'
/**
...
const animByMood: Record<string, any> = {
  sad: happyAnim,
  neutral: happyAnim,
  happy: happyAnim,
  excited: happyAnim,
...
function PetAvatar({ mood }: { mood: string }) {
  const lottieRef = useRef<any>(null)
  const { play, setDirection, playSegments, seek, pause, setDisplayRef } = useLottie({
    src: animByMood[mood] ?? happyAnim,
    loop: false,
    autoplay: false,
  })

  // Запустить idle анимацию при монтировании
  useEffect(() => {
    setDirection('forward')
    play()
  }, [play, setDirection])

  // Обработчик клика — проиграть действие и вернуться в idle
  const handleClick = () => {
    // Перейти к началу и остановиться
    seek(0)
    pause()

    // Воспроизвести сегмент действия (с 0 по 30 кадр)
    playSegments([0, 30], { queue: true })

    // После завершения действия вернуться в idle
    setTimeout(() => {
      setDirection('forward')
      play()
    }, 1500)
  ...
  return (
    <div onClick={handleClick} style={{ cursor: 'pointer' }}>
      <div ref={setDisplayRef} style={{ width: 160, height: 160 }} />
    </div>
  )
}
/*
