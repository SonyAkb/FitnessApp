...
Сквозной демо-сценарий важнее полного списка фич. Клиент — **Android (`mobile-fit-baddy`)**, сервер — **FastAPI (`backend-fit-baddy`)**. Raspberry Pi в историях не участвует: живые пульс и шаги в MVP даёт **симулятор** по тому же контракту.
...
Демо для экзаменатора:

**Login → сегодняшняя тренировка → complete → XP → питомец реагирует → дашборд → уведомление → живая телеметрия**
...
**чтобы** дальше видеть персональные тренировки и питомца.
...
**я хочу** войти и сразу увидеть, что делать сегодня  
**чтобы** не искать план по меню.
...
Если слотов нет — пустой экран «На сегодня ничего не запланировано», без выдуманных тренировок.
...
**чтобы** «сегодняшняя тренировка» откуда-то бралась.
...
**Как** пользователь на тренировке  
**я хочу** запускать сессию, отмечать упражнения и видеть таймер отдыха  
**чтобы** пройти программу без бумаги.
...
**чтобы** реальное усилие было наградой.
...
**чтобы** понимать прогресс без догадок.
...
**Как** пользователь на демо  
**я хочу** видеть пульс и шаги «как с часов»  
**чтобы** показать интеграцию устройства без Raspberry Pi.
...
**чтобы** за секунды понять, занимался ли я.
...
**Как** пользователь  
**я хочу** ленту напоминаний и реакций системы (тренировка засчитана, питомец грустит)  
**чтобы** не пропустить слот и увидеть обратную связь.
...
**Как** пользователь  
**я хочу** короткую рекомендацию по прогрессу  
**чтобы** понять, что поменять в следующем плане.
...
**я хочу** пройти демо-оплату Pro без Stripe  
**чтобы** на защите показать тариф и webhook.
...
**чтобы** экран не был пустым при падении сокета.
...
## Соответствие демо-сценарию

| Шаг экзаменатора | История |
|---|---|
| Пользователь входит | 2 |
| Открывает сегодняшнюю тренировку | 2, 3 |
| Стартует, отмечает упражнения, таймер | 4 |
| Завершает | 5 |
| Система записывает тренировку | 5 `(backend)` |
| Питомец получает XP | 5, 6 |
| Меняется mood / level | 5, 6 |
| Дашборд обновляется | 8 |
| Появляется уведомление | 9 |
| Видны пульс и шаги | 7, 12 |
